...
2. **Send via secure channel**:
   ...
   - Use the **<a href="https://go.developerportal.qa.haloplus.io/" target="_blank">Developer Portal</a>** to submit your public key.
...
```jsonc
{
  ...
  "aud_fingerprints": "Public key hash", # android only
  "ksk_pin": "ksk", # android only
  "refresh_token": false, # ios only
  "x-tid": "Terminal ID", # ios only
  "mcc": "Merchant Category Code", # ios only
  "mbn": "Your Business Name", # ios only
  "tpid": "Transaction Provider ID" # ios only
}
```
...
| ksk_pin | String | Optional (ANDROID Only). SHA-256 fingerprint of the KSK used to sign this JWT. |
| refresh_token | boolean | Optional (IOS Only). Whether this JWT is a refresh token. |
| x-tid | String      | Optional (IOS Only). The Terminal ID. |
| mcc   | String      | Optional (IOS Only). The Merchant Category Code. |
| mbn   | String      | Optional (IOS Only). The Merchant Business Name. |
| tpid  | String      | Optional (IOS Only). The Transaction Provider ID. |
