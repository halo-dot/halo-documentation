...
Get the status information of a transaction if you are a Halo.SDK integrator or are performing intent (excluding the constant merchant id transaction [flow](/docs/documentations/intents/transaction-app2app-integration-guide#approach-12-constant-merchant-id-transaction)) and deeplink transactions - otherwise known as app-to-app transactions.
...
```json title="Webhook JSON body response"
{
  "id": "string",
  ...
  "amount": "number",
  "currency": "string",
  "haloReference": "string",
  "deviceInstallationId": "string",
  "paymentProviderId"?: any,
  "paymentProviderFields"?: any
}
```

| Field | Description | Type |
| ---------------------------- | --------------------- | ---------------- |
| id | Refers to the consumer transaction id that the QrCode ID uses | String (UUID) |
| merchantReference | Unique transaction reference provided by Halo.SDK integrating app. | String |
| disposition | <p>The outcome of the transaction (e.g. Approved)<br/><br/></p><p>There are 5 possible values for <code>disposition</code>​:</p><ul><li><code>Approved</code>​ - this means we got an approved message back from the processor </li><li><code>Declined</code>​ - this means we received a decline from the processor</li><li><code>UnableToGoOnline</code>​ - this means we couldn't connect to the processor. Essentially a decline, but it tells us the decline is due to a network error, and not a card issue</li><li><code>Indeterminate</code>​ - this means we either received an error from the processor or no response. This means we are unable to determine the status of the transaction. Typically, we treat these as declines and then we attempt to void the transaction in the background, in the event that the processor got the message but we just didn't get the response</li><li><code>Voided</code>​ - the transaction was reversed.</li></ul> | String (32) |
...
| authorizationCode | Message Authorization Code (MAC) | String (MAC Length) |
| amount | Value of transaction (e.g 100.01) | Number |
| currency | Currency of the amount (e.g. ZAR, GBP) | String (3) |
| haloReference | The halo backend transaction id reference | String (UUID) |
| deviceInstallationId | This id identifies the installation of the app running the Halo.SDK | String |
| paymentProviderId | This is the id that identifies the merchant at the payment provider they're registered with | Object |
| paymentProviderFields | These are fields returned by the payment providers which may be used by the merchant e.g. recon references | Object |

#### Payment Provider Configuration

> **Important:** The structure of payment provider fields depends on the processor handling the transaction.

#### Payment Provider ID

The `Payment Provider ID` varies by processor.

**Postbridge processors:**
```json
{
  "tid": "string",
  "mid": "string"
...
These fields may also differ depending on the processor.
...
> Always confirm the expected structure before proceeding with integration.

Please contact **Halo** to verify the exact structure of both:
...
You need to supply one of the following authentication methods in the header of the API request:

| Name                                        | Type   | Description                                    |
| ------------------------------------------- | ------ | ---------------------------------------------- |
| x-api-key<mark style={{color:'red'}}>\*</mark> | String | The API Key retrieved from the Merchant Portal |
| Authorization: Bearer \<token\><mark style={{color:'red'}}>\*</mark> | String | A token issued by your backend |
