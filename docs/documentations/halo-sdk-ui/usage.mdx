...
| `currency` | `String?` | ISO currency code (e.g., `"ZAR"`, `"USD"`). Defaults to `"ZAR"` if `null`. Must be supported by Halo. |
...
:::note
If `showTransactionResult` is `false` in `HDConfig`, the SDK returns the result as soon as the transaction completes, without showing its own success/failure screens.
:::
